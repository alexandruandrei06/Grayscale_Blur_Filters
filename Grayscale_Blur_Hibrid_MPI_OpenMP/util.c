...
void write_photo(params *param, bmp_photo *photo){
	...
	for(int i = 0; i < photo->infoheader->height; i++){
		for(int j = 0; j < photo->infoheader->width; j++){
            int index = i * photo->infoheader->width + j;
			fwrite(&photo->bitmap[index].blue, sizeof(unsigned char), 1, output_fd);
			fwrite(&photo->bitmap[index].green, sizeof(unsigned char), 1, output_fd);
			fwrite(&photo->bitmap[index].red, sizeof(unsigned char), 1, output_fd);
		}
		if(padding < 4){
...
void read_bitmap(bmp_photo *photo, FILE *input_fd){
	...
	for(int i = 0; i < photo->infoheader->height; i++){
		for(int j = 0; j < photo->infoheader->width; j++){
            int index = i * photo->infoheader->width + j;
			fread(&photo->bitmap[index].blue, sizeof(unsigned char), 1, input_fd);
			fread(&photo->bitmap[index].green, sizeof(unsigned char), 1, input_fd);
			fread(&photo->bitmap[index].red, sizeof(unsigned char), 1, input_fd);
		}
		if(padding < 4)
